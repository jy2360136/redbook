...
## 简介
本项目提供一套面向金融与科技领域的自动化新闻采集工作流，覆盖7大权威财经/科技媒体，采用模块化设计，支持RSS源、API源、动态网页（Playwright）与传统HTTP抓取策略。系统提供统一的命令行接口、错误处理与性能优化策略，并输出结构化JSON数据，便于后续分析与内容生成。
...
```mermaid
graph TB
A["financial_news_workflow_crawl4ai.py<br/>主程序与各媒体源抓取器"] --> B["requirements.txt<br/>依赖清单"]
A --> C["docs/RUN.md<br/>运行说明"]
A --> D["test_all_sources.py<br/>媒体源测试"]
A --> E["test_crawl4ai.py<br/>Crawl4AI功能测试"]
A --> F["news_output_crawl4ai_*/news_result.json<br/>输出结果"]
```
...
## 核心组件
- 媒体源抓取器：针对7大媒体的专用抓取类，封装不同抓取策略与解析逻辑
- 主程序：统一调度、去重、保存与输出
...
- 可选增强：Crawl4AI（备用）与Scrapling（主力增强）可进一步提升成功率与稳定性
...
```mermaid
flowchart TD
Start(["开始"]) --> Fetch["调用各媒体源fetch()"]
Fetch --> Collect["收集新闻列表"]
Collect --> Dedup["去重按标题"]
Dedup --> Stats["统计来源分布"]
Stats --> Save["保存为news_result.json"]
Save --> End(["结束"])
```
...
- AI增强：crawl4ai、numpy、pillow、scipy、scikit-learn、nltk、rank-bm25、snowballstemmer、litellm、openai、tiktoken、tokenizers、huggingface-hub、aiosqlite
...
AI["AI增强"] --> C4AI["crawl4ai"]
AI --> ML["numpy/pillow/scipy/scikit-learn/nltk/rank-bm25/snowballstemmer"]
AI --> LLM["litellm/openai/tiktoken/tokenizers/huggingface-hub"]
AI --> DB["aiosqlite"]
...
## 结论
该工作流通过模块化设计与多策略适配，实现了对7大权威财经媒体的高效自动化抓取。系统具备完善的错误处理、性能优化与输出规范，适合在金融与科技领域进行热点监测、选题策划与舆情分析。
