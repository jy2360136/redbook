...
- community_crawler.py：社区论坛信息抓取工具，支持雪球、知乎等社区，提供统一抓取入口与保存功能
- test_all_sources.py：测试脚本，验证各媒体源连通性与解析能力
- requirements.txt：完整依赖清单，包含 feedparser、requests、playwright、beautifulsoup4 等
...
K["test_all_sources.py<br/>媒体源测试"] --> D
L["requirements.txt<br/>依赖清单"] --> A
L --> H
M["docs/RUN.md<br/>运行文档"] --> G
N["示例输出<br/>news_result.json"] --> F
```
...
- 社区抓取工具：提供 fetch_all_communities 与 save_comments，支持情感分析
